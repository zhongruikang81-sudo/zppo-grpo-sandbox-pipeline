...
def prepare_dpo_dataset(data_path, tokenizer):
    """
    读取 JSONL 数据并进行格式化。
    使用 Gemma-2 的 Chat Template 包装 prompt，并在 chosen 和 rejected 结尾添加 EOS token。
    """
    ...
    for line in lines:
        ...
        # 1. 包装 User 提示词 (应用 Gemma-2 Chat Template 并带上 generation prompt)
        ...
        # 2. 补全 chosen 和 rejected，并在末尾加上 EOS token
...
def main():
    parser = argparse.ArgumentParser(description="Run DPO alignment to suppress repetition in local Gemma-2 model")
    ...
    parser.add_argument("--checkpoint-path", default=str(REPO_ROOT / "grpo_output_discounted" / "checkpoint_step230_v2"), 
                        help="Path to the checkpoint adapter to fine-tune")
    parser.add_argument("--dpo-data-path", default=str(REPO_ROOT / "data" / "dpo_anti_repetition.jsonl"), 
                        help="Path to the generated DPO dataset JSONL")
    parser.add_argument("--output-dir", default=str(REPO_ROOT / "grpo_output_discounted" / "checkpoint_step230_v2_dpo"), 
    ...
    # Note: Using batch_size=1 and accumulation=8 to fit in 8GB VRAM
    ...
    # Setting ref_model=None will automatically disable LoRA adapters for computing reference logps (saves massive VRAM)
